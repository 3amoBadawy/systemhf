    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">تعديل الدور: {{ $role->name_ar }}</h1>
                    <p class="mt-2 text-gray-600">قم بتعديل معلومات الدور والصلاحيات</p>
                </div>
            ...
            <form action="{{ route('roles.update', $role) }}" method="POST" class="p-6">
                ...
                <div class="mb-8">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="وصف مختصر للدور والصلاحيات الممنوحة">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
